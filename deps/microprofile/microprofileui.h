...
void MicroProfileDrawDetailedBars(uint32_t nWidth, uint32_t nHeight, int nBaseY, int nSelectedFrame)
{
	...
	if(!bSkipBarView)
	{
		for(uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i)
		{
			...
			snprintf(ThreadName, sizeof(ThreadName)-1, "%04x %s", (uint32_t)nThreadId, &pLog->ThreadName[0] );
			nY += 3;
...
#define MICROPROFILE_MENU_MAX 16
	...
	{	[] (int index, bool& bSelected) -> const char*{
		...
		[] (int index, bool& bSelected) -> const char*{
			MicroProfile& S = *MicroProfileGet();			
			if(index < (int)(sizeof(g_MicroProfileAggregatePresets)/sizeof(g_MicroProfileAggregatePresets[0])))
			{
