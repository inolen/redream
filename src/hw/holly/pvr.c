...
static uint8_t pvr_vram_interleaved_r8(pvr_t *pvr, uint32_t addr);
static uint16_t pvr_vram_interleaved_r16(pvr_t *pvr, uint32_t addr);
static uint32_t pvr_vram_interleaved_r32(pvr_t *pvr, uint32_t addr);

static void pvr_vram_interleaved_w8(pvr_t *pvr, uint32_t addr, uint8_t value);
static void pvr_vram_interleaved_w16(pvr_t *pvr, uint32_t addr, uint16_t value);
static void pvr_vram_interleaved_w32(pvr_t *pvr, uint32_t addr, uint32_t value);

DECLARE_REG_W32(pvr_t *pvr, SPG_LOAD);
DECLARE_REG_W32(pvr_t *pvr, FB_R_CTRL);
...
void pvr_reconfigure_spg(pvr_t *pvr) {
...
void pvr_next_scanline(pvr_t *pvr) {
...
uint32_t pvr_reg_r32(pvr_t *pvr, uint32_t addr) {
...
void pvr_reg_w32(pvr_t *pvr, uint32_t addr, uint32_t value) {
  uint32_t offset = addr >> 2;
...
#define define_vram_interleaved_read(name, type)                \
  type pvr_vram_interleaved_##name(pvr_t *pvr, uint32_t addr) { \
    addr = MAP64(addr);                                         \
...
#define define_vram_interleaved_write(name, type)                           \
  void pvr_vram_interleaved_##name(pvr_t *pvr, uint32_t addr, type value) { \
    addr = MAP64(addr);                                                     \
