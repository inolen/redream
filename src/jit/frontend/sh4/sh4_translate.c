...
#ifndef NDEBUG
  return ir_load_slow(ir, add, type);
#else
  return (flags & JIT_SLOWMEM) ? ir_load_slow(ir, addr, type)
...
static struct ir_value *load_gpr(struct ir *ir, int n, enum ir_type type) {
  int offset = offsetof(struct sh4_ctx, r[n]);
  return ir_load_context(ir, offset, type);
}

static void store_gpr(struct ir *ir, int n, struct ir_value *v) {
  CHECK_EQ(v->type, VALUE_I32);
  int offset = offsetof(struct sh4_ctx, r[n]);
  ir_store_context(ir, offset, v);
}

static struct ir_value *load_fpr(struct ir *ir, int n, enum ir_type type) {
  int offset = offsetof(struct sh4_ctx, fr[swizzle_fpr(n, type)]);
  return ir_load_context(ir, offset, type);
}

static void store_fpr(struct ir *ir, int n, struct ir_value *v) {
  int offset = offsetof(struct sh4_ctx, fr[swizzle_fpr(n, v->type)]);
  ir_store_context(ir, offset, v);
}

static struct ir_value *load_xfr(struct ir *ir, int n, enum ir_type type) {
  int offset = offsetof(struct sh4_ctx, xf[swizzle_fpr(n, type)]);
  return ir_load_context(ir, offset, type);
}

static void store_xfr(struct ir *ir, int n, struct ir_value *v) {
...
static void store_sr(struct ir *ir, struct ir_value *v, int update) {
  CHECK_EQ(v->type, VALUE_I32);
...
static struct ir_value *load_t(struct ir *ir) {
  return ir_and(ir, load_sr(ir), ir_alloc_i32(ir, T));
}

static void store_t(struct ir *ir, struct ir_value *v) {
  struct ir_value *sr = load_sr(ir);
  ...
  store_sr(ir, res, false);
}

static struct ir_value *load_gbr(struct ir *ir) {
  return ir_load_context(ir, offsetof(struct sh4_ctx, gbr), VALUE_I32);
}

static void store_gbr(struct ir *ir, struct ir_value *v) {
...
static void store_fpscr(struct ir *ir, struct ir_value *v) {
  CHECK_EQ(v->type, VALUE_I32);
  v = ir_and(ir, v, ir_alloc_i32(ir, 0x003fffff));

  struct ir_value *fpscr_updated =
      ir_load_context(ir, offsetof(struct sh4_ctx, FPSCRUpdated), VALUE_I64);
...
static struct ir_value *load_pr(struct ir *ir) {
  return ir_load_context(ir, offsetof(struct sh4_ctx, pr), VALUE_I32);
}

static void store_pr(struct ir *ir, struct ir_value *v) {
  CHECK_EQ(v->type, VALUE_I32);
  ir_store_context(ir, offsetof(struct sh4_ctx, pr), v);
}

static void branch(struct ir *ir, struct ir_value *addr) {
  ir_store_context(ir, offsetof(struct sh4_ctx, pc), addr);
}

static void branch_cond(struct ir *ir, struct ir_value *cond,
                        struct ir_value *addr_true,
                        struct ir_value *addr_false) {
...
EMITTER(INVALID) {
  ...
  ir_call_external_2(ir, invalid_instruction,
                     ir_alloc_i64(ir, (int64_t)i->addr));
}
...
EMITTER(MOVWLPC) {
  ...
  struct ir_value *v = load_guest(ir, flags, ir_alloc_i32(ir, addr), VALUE_I16);
...
EMITTER(MOVLLPC) {
  ...
  struct ir_value *v = load_guest(ir, flags, ir_alloc_i32(ir, addr), VALUE_I32);
  store_gpr(ir, i->Rn, v);
}

// MOV     Rm,Rn
EMITTER(MOV) {
  struct ir_value *v = load_gpr(ir, i->Rm, VALUE_I32);
  store_gpr(ir, i->Rn, v);
}

// MOV.B   Rm,@Rn
EMITTER(MOVBS) {
  struct ir_value *addr = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *v = load_gpr(ir, i->Rm, VALUE_I8);
  store_guest(ir, flags, addr, v);
}

// MOV.W   Rm,@Rn
EMITTER(MOVWS) {
  struct ir_value *addr = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *v = load_gpr(ir, i->Rm, VALUE_I16);
  store_guest(ir, flags, addr, v);
}
...
EMITTER(MOVBL) {
  struct ir_value *v =
      load_guest(ir, flags, load_gpr(ir, i->Rm, VALUE_I32), VALUE_I8);
  v = ir_sext(ir, v, VALUE_I32);
  store_gpr(ir, i->Rn, v);
}
...
EMITTER(MOVWL) {
  struct ir_value *v =
      load_guest(ir, flags, load_gpr(ir, i->Rm, VALUE_I32), VALUE_I16);
  v = ir_sext(ir, v, VALUE_I32);
  store_gpr(ir, i->Rn, v);
}
...
EMITTER(MOVLL) {
  struct ir_value *v =
      load_guest(ir, flags, load_gpr(ir, i->Rm, VALUE_I32), VALUE_I32);
  store_gpr(ir, i->Rn, v);
}
...
EMITTER(MOVBP) {
  /* store (Rm) at Rn */
  struct ir_value *addr = load_gpr(ir, i->Rm, VALUE_I32);
  struct ir_value *v = load_guest(ir, flags, addr, VALUE_I8);
  v = ir_sext(ir, v, VALUE_I32);
  store_gpr(ir, i->Rn, v);
...
EMITTER(MOVWP) {
  /* store (Rm) at Rn */
  struct ir_value *addr = load_gpr(ir, i->Rm, VALUE_I32);
  struct ir_value *v = load_guest(ir, flags, addr, VALUE_I16);
  v = ir_sext(ir, v, VALUE_I32);
  store_gpr(ir, i->Rn, v);
...
EMITTER(MOVLP) {
  /* store (Rm) at Rn */
  struct ir_value *addr = load_gpr(ir, i->Rm, VALUE_I32);
  struct ir_value *v = load_guest(ir, flags, addr, VALUE_I32);
  store_gpr(ir, i->Rn, v);
...
EMITTER(MOVBS0D) {
  ...
  struct ir_value *v = load_gpr(ir, 0, VALUE_I8);
  store_guest(ir, flags, addr, v);
}
...
EMITTER(MOVWS0D) {
  ...
  struct ir_value *v = load_gpr(ir, 0, VALUE_I16);
  store_guest(ir, flags, addr, v);
}
...
// MOV.B   Rm,@(R0,Rn)
EMITTER(MOVBS0) {
  struct ir_value *addr = load_gpr(ir, 0, VALUE_I32);
  addr = ir_add(ir, addr, load_gpr(ir, i->Rn, VALUE_I32));
  struct ir_value *v = load_gpr(ir, i->Rm, VALUE_I8);
  store_guest(ir, flags, addr, v);
}
...
EMITTER(MOVBL0) {
  struct ir_value *addr = load_gpr(ir, 0, VALUE_I32);
  addr = ir_add(ir, addr, load_gpr(ir, i->Rm, VALUE_I32));
  struct ir_value *v =
      ir_sext(ir, load_guest(ir, flags, addr, VALUE_I8), VALUE_I32);
  store_gpr(ir, i->Rn, v);
}
...
// MOV.B   R0,@(disp,GBR)
EMITTER(MOVBS0G) {
  struct ir_value *addr = load_gbr(ir);
  addr = ir_add(ir, addr, ir_alloc_i32(ir, i->disp));
  struct ir_value *v = load_gpr(ir, 0, VALUE_I8);
  store_guest(ir, flags, addr, v);
}
...
EMITTER(MOVLS0G) {
  struct ir_value *addr = load_gbr(ir);
  addr = ir_add(ir, addr, ir_alloc_i32(ir, i->disp * 4));
  struct ir_value *v = load_gpr(ir, 0, VALUE_I32);
  store_guest(ir, flags, addr, v);
}
...
EMITTER(MOVA) {
  uint32_t addr = (i->disp * 4) + (i->addr & ~3) + 4;
  store_gpr(ir, 0, ir_alloc_i32(ir, addr));
}

// MOVT    Rn
EMITTER(MOVT) {
  store_gpr(ir, i->Rn, load_t(ir));
}
...
// XTRCT   Rm,Rn
EMITTER(XTRCT) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  rn = ir_lshri(ir, ir_and(ir, rn, ir_alloc_i32(ir, 0xffff0000)), 16);
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I32);
  rm = ir_shli(ir, ir_and(ir, rm, ir_alloc_i32(ir, 0x0000ffff)), 16);
  store_gpr(ir, i->Rn, ir_or(ir, rn, rm));
}
...
EMITTER(CMPEQI) {
  struct ir_value *imm = ir_alloc_i32(ir, (int32_t)(int8_t)i->imm);
  struct ir_value *r0 = load_gpr(ir, 0, VALUE_I32);
  store_t(ir, ir_cmp_eq(ir, r0, imm));
}
...
EMITTER(CMPHS) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I32);
  store_t(ir, ir_cmp_uge(ir, rn, rm));
}
...
EMITTER(CMPGE) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I32);
  store_t(ir, ir_cmp_sge(ir, rn, rm));
}
...
EMITTER(CMPHI) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I32);
  store_t(ir, ir_cmp_ugt(ir, rn, rm));
}
...
EMITTER(CMPGT) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I32);
  store_t(ir, ir_cmp_sgt(ir, rn, rm));
}
...
EMITTER(CMPPZ) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  store_t(ir, ir_cmp_sge(ir, rn, ir_alloc_i32(ir, 0)));
}
...
EMITTER(CMPPL) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  store_t(ir, ir_cmp_sgt(ir, rn, ir_alloc_i32(ir, 0)));
}
...
EMITTER(CMPSTR) {
  ...
  store_t(ir, ir_or(ir, ir_or(ir, ir_or(ir, b1_eq, b2_eq), b3_eq), b4_eq));
}
...
EMITTER(DIV0U) {
  ...
  store_sr(ir, ir_and(ir, load_sr(ir), ir_alloc_i32(ir, ~T)), false);
}
...
// DMULS.L Rm,Rn
EMITTER(DMULS) {
  struct ir_value *rn = ir_sext(ir, load_gpr(ir, i->Rn, VALUE_I32), VALUE_I64);
  struct ir_value *rm = ir_sext(ir, load_gpr(ir, i->Rm, VALUE_I32), VALUE_I64);
...
// DMULU.L Rm,Rn
EMITTER(DMULU) {
  struct ir_value *rn = ir_zext(ir, load_gpr(ir, i->Rn, VALUE_I32), VALUE_I64);
  struct ir_value *rm = ir_zext(ir, load_gpr(ir, i->Rm, VALUE_I32), VALUE_I64);
...
EMITTER(DT) {
  ...
  store_gpr(ir, i->Rn, v);
  store_t(ir, ir_cmp_eq(ir, v, ir_alloc_i32(ir, 0)));
}

// EXTS.B  Rm,Rn
EMITTER(EXTSB) {
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I8);
...
// EXTS.W  Rm,Rn
EMITTER(EXTSW) {
  struct ir_value *rm = load_gpr(ir, i->Rm, VALUE_I16);
  struct ir_value *v = ir_sext(ir, rm, VALUE_I32);
  store_gpr(ir, i->Rn, v);
}
...
// MULS    Rm,Rn
EMITTER(MULS) {
  struct ir_value *rn = ir_sext(ir, load_gpr(ir, i->Rn, VALUE_I16), VALUE_I32);
  struct ir_value *rm = ir_sext(ir, load_gpr(ir, i->Rm, VALUE_I16), VALUE_I32);
  struct ir_value *v = ir_smul(ir, rn, rm);
  ir_store_context(ir, offsetof(struct sh4_ctx, macl), v);
}
...
// MULU    Rm,Rn
EMITTER(MULU) {
  struct ir_value *rn = ir_zext(ir, load_gpr(ir, i->Rn, VALUE_I16), VALUE_I32);
  struct ir_value *rm = ir_zext(ir, load_gpr(ir, i->Rm, VALUE_I16), VALUE_I32);
...
EMITTER(ANDB) {
  struct ir_value *addr = load_gpr(ir, 0, VALUE_I32);
  addr = ir_add(ir, addr, load_gbr(ir));
  struct ir_value *v = load_guest(ir, flags, addr, VALUE_I8);
...
EMITTER(TAS) {
  struct ir_value *addr = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *v = load_guest(ir, flags, addr, VALUE_I8);
  store_guest(ir, flags, addr, ir_or(ir, v, ir_alloc_i8(ir, 0x80)));
  store_t(ir, ir_cmp_eq(ir, v, ir_alloc_i8(ir, 0)));
}
...
EMITTER(TSTB) {
  struct ir_value *addr = load_gpr(ir, 0, VALUE_I32);
  addr = ir_add(ir, addr, load_gbr(ir));
  struct ir_value *data = load_guest(ir, flags, addr, VALUE_I8);
...
EMITTER(ROTCR) {
  struct ir_value *rn = load_gpr(ir, i->Rn, VALUE_I32);
  struct ir_value *rn_lsb = ir_and(ir, rn, ir_alloc_i32(ir, 0x1));
  struct ir_value *v = ir_shli(ir, load_t(ir), 31);
  v = ir_or(ir, v, ir_lshri(ir, rn, 1));
  store_gpr(ir, i->Rn, v);
  store_t(ir, rn_lsb);
}
...
EMITTER(BF) {
  ...
  struct ir_value *cond = load_t(ir);
  branch_cond(ir, cond, ir_alloc_i32(ir, i->addr + 2),
...
EMITTER(BFS) {
  ...
  emit_delay_instr();
  uint32_t dest_addr = ((int8_t)i->disp * 2) + i->addr + 4;
  branch_cond(ir, cond, ir_alloc_i32(ir, i->addr + 4),
              ir_alloc_i32(ir, dest_addr));
}
...
EMITTER(BT) {
  uint32_t dest_addr = ((int8_t)i->disp * 2) + i->addr + 4;
  struct ir_value *cond = load_t(ir);
  branch_cond(ir, cond, ir_alloc_i32(ir, dest_addr),
...
EMITTER(BRA) {
  ...
  branch(ir, ir_alloc_i32(ir, dest_addr));
}
...
EMITTER(BSR) {
  ...
  store_pr(ir, ir_alloc_i32(ir, ret_addr));
  branch(ir, ir_alloc_i32(ir, dest_addr));
}
...
EMITTER(BSRF) {
  ...
  store_pr(ir, ret_addr);
  branch(ir, dest_addr);
}

// JMP     @Rm
EMITTER(JMP) {
  struct ir_value *dest_addr = load_gpr(ir, i->Rn, VALUE_I32);
  emit_delay_instr();
  branch(ir, dest_addr);
}
...
// RTS
EMITTER(RTS) {
  struct ir_value *dest_addr = load_pr(ir);
  emit_delay_instr();
  branch(ir, dest_addr);
}
...
EMITTER(LDCMSR) {
  ...
  addr = load_gpr(ir, i->Rm, VALUE_I32);
  addr = ir_add(ir, addr, ir_alloc_i32(ir, 4));
  store_gpr(ir, i->Rm, addr);
}
...
EMITTER(LDCMRBANK) {
  int reg = i->Rn & 0x7;
  struct ir_value *addr = load_gpr(ir, i->Rm, VALUE_I32);
  store_gpr(ir, i->Rm, ir_add(ir, addr, ir_alloc_i32(ir, 4)));
  struct ir_value *v = load_guest(ir, flags, addr, VALUE_I32);
...
EMITTER(PREF) {
  ...
  struct ir_value *addr =
      ir_zext(ir, load_gpr(ir, i->Rn, VALUE_I32), VALUE_I64);
...
// SETS
EMITTER(SETS) {
  struct ir_value *sr = ir_or(ir, load_sr(ir), ir_alloc_i32(ir, S));
  store_sr(ir, sr, true);
}

// SETT
EMITTER(SETT) {
  store_t(ir, ir_alloc_i32(ir, 1));
}
...
// STC     SR,Rn
EMITTER(STCSR) {
  struct ir_value *v = load_sr(ir);
  store_gpr(ir, i->Rn, v);
}

// STC     GBR,Rn
EMITTER(STCGBR) {
  struct ir_value *v = load_gbr(ir);
  store_gpr(ir, i->Rn, v);
}
...
EMITTER(STCMSR) {
  struct ir_value *addr =
      ir_sub(ir, load_gpr(ir, i->Rn, VALUE_I32), ir_alloc_i32(ir, 4));
  store_gpr(ir, i->Rn, addr);
  struct ir_value *v = load_sr(ir);
  store_guest(ir, flags, addr, v);
}
...
EMITTER(STSMMACH) {
  ...
  struct ir_value *mach =
      ir_load_context(ir, offsetof(struct sh4_ctx, mach), VALUE_I32);
  store_guest(ir, flags, addr, mach);
}
...
EMITTER(STSMMACL) {
  ...
  struct ir_value *macl =
      ir_load_context(ir, offsetof(struct sh4_ctx, macl), VALUE_I32);
  store_guest(ir, flags, addr, macl);
}
...
EMITTER(STSMPR) {
  ...
  struct ir_value *pr = load_pr(ir);
  store_guest(ir, flags, addr, pr);
}
...
EMITTER(FLDI0) {
  store_fpr(ir, i->Rn, ir_alloc_i32(ir, 0));
}

// FLDI1  FRn 1111nnnn10011101
EMITTER(FLDI1) {
  store_fpr(ir, i->Rn, ir_alloc_i32(ir, 0x3F800000));
}
...
EMITTER(FMOV) {
  if (flags & SH4_DOUBLE_SZ) {
    if (i->Rm & 1) {
      struct ir_value *rm = load_xfr(ir, i->Rm & 0xe, VALUE_I64);
      if (i->Rn & 1) {
        store_xfr(ir, i->Rn & 0xe, rm);
      } else {
        store_fpr(ir, i->Rn, rm);
      }
    } else {
      struct ir_value *rm = load_fpr(ir, i->Rm, VALUE_I64);
      if (i->Rn & 1) {
        store_xfr(ir, i->Rn & 0xe, rm);
      } else {
        store_fpr(ir, i->Rn, rm);
    ...
    store_fpr(ir, i->Rn, load_fpr(ir, i->Rm, VALUE_I32));
  }
}
...
EMITTER(FMOV_LOAD) {
  ...
  if (flags & SH4_DOUBLE_SZ) {
    struct ir_value *v_low = load_guest(ir, flags, addr, VALUE_I32);
    struct ir_value *v_high =
        load_guest(ir, flags, ir_add(ir, addr, ir_alloc_i32(ir, 4)), VALUE_I32);
    if (i->Rn & 1) {
      store_xfr(ir, i->Rn & 0xe, v_low);
      store_xfr(ir, i->Rn, v_high);
    } else {
      store_fpr(ir, i->Rn, v_low);
      store_fpr(ir, i->Rn | 0x1, v_high);
    }
  } else {
    store_fpr(ir, i->Rn, load_guest(ir, flags, addr, VALUE_I32));
  }
}
...
EMITTER(FMOV_INDEX_LOAD) {
  struct ir_value *addr =
      ir_add(ir, load_gpr(ir, 0, VALUE_I32), load_gpr(ir, i->Rm, VALUE_I32));
...
EMITTER(FMOV_STORE) {
  ...
  if (flags & SH4_DOUBLE_SZ) {
    ...
    if (i->Rm & 1) {
      store_guest(ir, flags, addr_low, load_xfr(ir, i->Rm & 0xe, VALUE_I32));
      store_guest(ir, flags, addr_high, load_xfr(ir, i->Rm, VALUE_I32));
    } else {
      store_guest(ir, flags, addr_low, load_fpr(ir, i->Rm, VALUE_I32));
      store_guest(ir, flags, addr_high, load_fpr(ir, i->Rm | 0x1, VALUE_I32));
    }
  } else {
    store_guest(ir, flags, addr, load_fpr(ir, i->Rm, VALUE_I32));
  }
}
...
EMITTER(FMOV_INDEX_STORE) {
  struct ir_value *addr =
      ir_add(ir, load_gpr(ir, 0, VALUE_I32), load_gpr(ir, i->Rn, VALUE_I32));
...
EMITTER(FMOV_SAVE) {
  if (flags & SH4_DOUBLE_SZ) {
    struct ir_value *addr =
        ir_sub(ir, load_gpr(ir, i->Rn, VALUE_I32), ir_alloc_i32(ir, 8));
    store_gpr(ir, i->Rn, addr);
...
EMITTER(FMOV_RESTORE) {
  ...
  if (flags & SH4_DOUBLE_SZ) {
    ...
    store_gpr(ir, i->Rm, ir_add(ir, addr, ir_alloc_i32(ir, 8)));
  } else {
    store_fpr(ir, i->Rn, load_guest(ir, flags, addr, VALUE_I32));
    store_gpr(ir, i->Rm, ir_add(ir, addr, ir_alloc_i32(ir, 4)));
  }
...
EMITTER(FLDS) {
  struct ir_value *rn = load_fpr(ir, i->Rm, VALUE_I32);
...
EMITTER(FABS) {
  if (flags & SH4_DOUBLE_PR) {
    int n = i->Rn & 0xe;
    struct ir_value *v = ir_fabs(ir, load_fpr(ir, n, VALUE_F64));
    store_fpr(ir, n, v);
  } else {
    struct ir_value *v = ir_fabs(ir, load_fpr(ir, i->Rn, VALUE_F32));
    store_fpr(ir, i->Rn, v);
  }
}
...
EMITTER(FSRRA) {
  struct ir_value *frn = load_fpr(ir, i->Rn, VALUE_F32);
...
EMITTER(FADD) {
  if (flags & SH4_DOUBLE_PR) {
    int n = i->Rn & 0xe;
    int m = i->Rm & 0xe;
    struct ir_value *drn = load_fpr(ir, n, VALUE_F64);
    struct ir_value *drm = load_fpr(ir, m, VALUE_F64);
    struct ir_value *v = ir_fadd(ir, drn, drm);
    store_fpr(ir, n, v);
  } else {
    struct ir_value *frn = load_fpr(ir, i->Rn, VALUE_F32);
    struct ir_value *frm = load_fpr(ir, i->Rm, VALUE_F32);
...
EMITTER(FMAC) {
  ...
  struct ir_value *frn = load_fpr(ir, i->Rn, VALUE_F32);
  struct ir_value *frm = load_fpr(ir, i->Rm, VALUE_F32);
  struct ir_value *fr0 = load_fpr(ir, 0, VALUE_F32);
...
EMITTER(FCNVDS) {
  ...
  int m = i->Rm & 0xe;
  struct ir_value *dpv = load_fpr(ir, m, VALUE_F64);
...
// STS     FPSCR,Rn
EMITTER(STSFPSCR) {
  struct ir_value *fpscr = load_fpscr(ir);
  store_gpr(ir, i->Rn, fpscr);
}
...
EMITTER(STSMFPSCR) {
  ...
  addr = ir_sub(ir, addr, ir_alloc_i32(ir, 4));
  store_gpr(ir, i->Rn, addr);
  store_guest(ir, flags, addr, load_fpscr(ir));
}
...
EMITTER(STSMFPUL) {
  ...
  struct ir_value *fpul =
      ir_load_context(ir, offsetof(struct sh4_ctx, fpul), VALUE_I32);
  store_guest(ir, flags, addr, fpul);
}
...
EMITTER(FIPR) {
  ...
  struct ir_value *fvn = load_fpr(ir, n, VALUE_V128);
  struct ir_value *fvm = load_fpr(ir, m, VALUE_V128);
...
EMITTER(FSCA) {
  ...
  store_fpr(ir, n, ir_load(ir, addr, VALUE_F32));
  store_fpr(ir, n + 1,
...
EMITTER(FTRV) {
  ...
  struct ir_value *col0 = load_xfr(ir, 0, VALUE_V128);
  struct ir_value *row0 = ir_vbroadcast(ir, load_fpr(ir, n + 0, VALUE_F32));
  struct ir_value *result = ir_vmul(ir, col0, row0, VALUE_F32);

  struct ir_value *col1 = load_xfr(ir, 4, VALUE_V128);
  struct ir_value *row1 = ir_vbroadcast(ir, load_fpr(ir, n + 1, VALUE_F32));
  result = ir_vadd(ir, result, ir_vmul(ir, col1, row1, VALUE_F32), VALUE_F32);

  struct ir_value *col2 = load_xfr(ir, 8, VALUE_V128);
  struct ir_value *row2 = ir_vbroadcast(ir, load_fpr(ir, n + 2, VALUE_F32));
  result = ir_vadd(ir, result, ir_vmul(ir, col2, row2, VALUE_F32), VALUE_F32);

  struct ir_value *col3 = load_xfr(ir, 12, VALUE_V128);
  struct ir_value *row3 = ir_vbroadcast(ir, load_fpr(ir, n + 3, VALUE_F32));
  result = ir_vadd(ir, result, ir_vmul(ir, col3, row3, VALUE_F32), VALUE_F32);

  store_fpr(ir, n, result);
}
...
void sh4_translate(const struct jit_guest *guest, uint32_t addr, int size,
  ...
  if (tail_instr->op != OP_STORE_CONTEXT ||
    ...
    branch(ir, ir_alloc_i32(ir, addr + i));
  }
